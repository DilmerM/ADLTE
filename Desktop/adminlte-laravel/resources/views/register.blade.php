...
document.addEventListener('DOMContentLoaded', function () {
    ...
    registerForm.addEventListener('submit', function (event) {
        ...
        // La URL ahora apunta al endpoint correcto: /api/usuarios/registro
        // ==================================================================
        ...
        .then(response => {
            // Se añade un mejor manejo de la respuesta para dar errores más claros
            if (!response.ok) {
        ...
        .then(result => {
            if (result.success) {
                successMessageDiv.textContent = result.message + ' Redirigiendo al login...';
                successMessageDiv.classList.remove('d-none');
                setTimeout(() => { window.location.href = "{{ route('login') }}"; }, 2500);
            } else {
                // Si la API devuelve success: false, se muestra el mensaje
